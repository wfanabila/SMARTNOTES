...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        mysqli_query($conn, "DELETE FROM otp_verification WHERE email='$email' AND role='admin'");
        mysqli_query($conn, "INSERT INTO otp_verification (email, role, otp_code, expires_at) VALUES ('$email', 'admin', '$otp', '$expiresAt')");

        $sendResult = sendOTPEmail($email, $admin['adminName'], $otp, 'admin');

        if ($sendResult['success']) {
            $_SESSION['reset_email'] = $email;
            $_SESSION['reset_role']  = 'admin';
            $_SESSION['reset_name']  = $admin['adminName'];
        ...
        $error_msg = "No admin account found with that email.";
    }
...
<head>
    ...
    <title>UiTMNoteLink - Password Recovery (Admin)</title>
...
<body>
    ...
    <div class="login-wrapper">
        <div class="login-card">
            ...
            <h1 class="login-title">Password Recovery<br>(Admin)</h1>
            <p class="login-subtitle">Please insert your email and we will send<br>an OTP code</p>
            ...
            <form action="forgot_password_admin.php" method="post">
